...
const templateStr = `// Code generated by gen/gen.go; DO NOT EDIT.
package scanner

import "unsafe"

func (s *Scanner) scan{{.Name}}Unrolled(data []byte, min, max {{.Type.Type}}, offset int, results []int) []int {
	n := len(data)
	if n < {{.Type.Size}} {
		return results
	}
	...
	for ; i <= n-{{.Step}}; i += {{.Step}} {
		...
		if v{{.Index}} >= min && v{{.Index}} < max {
			results = append(results, offset+i+{{.Offset}})
		}
		{{- end}}
	}
...
func main() {
	...
	for _, cfg := range configs {
		...
		fileName := fmt.Sprintf("../scanner_%s.go", cfg.Type)
